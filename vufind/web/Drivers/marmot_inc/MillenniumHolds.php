		...
		if ($numMatches > 0 && $itemMatches == 0){
			...
			if (preg_match('/success/', $cleanResponse) && preg_match('/request denied/', $cleanResponse) == 0){
				...
				if (empty($reason)){
					...
					$hold_result['message'] = 'Received unrecognized response from the circulation system.';
...
//				if ($success) $logger->log('Thawed ok', PEAR_LOG_INFO);
			...
			elseif ($freezeValue == 'on'){
				...
				$success = empty($failure_messages);
				if ($success){
					$logger->log('Froze Hold ok', PEAR_LOG_INFO);
				}
...
//								$curHold['status'] = 'Ready';
			...
			if (isset($curHold['status'])){
				if ($curHold['status'] == 'Pending'){
					...
					if (preg_match('/\d+ Of (\d+) Holds?/i', $curHold['status'], $matches)){
						if ($matches[1] == 1){
							$curHold['freezeable'] = false;
						}
					}
	...
	function placeItemHold($patron, $recordId, $itemId, $pickupBranch, $cancelDate = null){
		...
		if ($loginResult){
			...
			if ($sResult){
				$sResult    = preg_replace("/<!--([^(-->)]*)-->/", "", $sResult);//Parse the response to get the status message
				...
				$holdResult = array(
					'success' => false,
					'message' => 'Did not receive a response from the circulation system.  Please try again in a few minutes.',
				);
			}
