 ...
 *
 * memcache keys
 * All keys are stored in memcache using this scheme (XXX = patron id):
...
class Sierra extends PatronDriverInterface {
    ...
    public function getPatron($uid = null) {
        ...
        $pObjKey = 'patron_' . $uid . '_obj';
        if ($pObj = $this->memCache->get($pObjKey)) {
            return $pObj;
        }
        ...
        // check patron type
        if($pInfo->patronType != $patron->pType) {
            $updatePatron = true;
            $patron->pType = $pInfo->patronType;
        }
        // check email
        ...
        // TODO: Need to figure out which phone to use.
        if((isset($homePhone) && $patron->phone != $homePhone) ||
          (isset($mobilePhone) && $mobilePhone != $patron->phone)) {
            $updatePatron = true;
            $patron->phone = isset($homePhone) ? $homePhone : $mobilePhone;
        }
        // check home location
        ...
        if($patron->address2 != '') {
            ...
            $stateZipParts = explode(' ', $addressParts[1]);
        ...
        try {
            ...
            $dateDiff    = $expiresDate->diff($nowDate);
            $expiresInDays = $dateDiff->format('%r%d');
            if((int) $expiresInDays <= 30) {
                $patron->expireClose = 1;
            } else {
                $patron->expireClose = 0;
            }
            if((int) $expiresInDays < 0) {
                $patron->expired = 1;
            } else {
                $patron->expired = 0;
            }
