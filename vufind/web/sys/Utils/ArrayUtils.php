...
class ArrayUtils
{
    ...
    {
        return self::recursiveUtf8Encode($array);
    }

    /**
     * Internal helper that recursively encodes all keys/values as UTF-8.
     */
    private static function recursiveUtf8Encode(array $array): array
    {
        $encodedArray = [];
        $utf8 = 'UTF-8';
        ...
        foreach($array as $key => $value) {
            ...
            if(is_string($key) && !mb_check_encoding($key, $utf8)) {
                $key = mb_convert_encoding($key, $utf8, $possibleEncodings);
            }

            // Recurse or encode the value
            if(is_array($value)) {
                // Recurse for sub-arrays
                $encodedArray[$key] = self::recursiveUtf8Encode($value);
            } elseif(is_string($value)) {
                // Encode strings
                $encodedArray[$key] = mb_check_encoding($value, $utf8)
                    ? $value
