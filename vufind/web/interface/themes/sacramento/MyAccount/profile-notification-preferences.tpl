			...
			{if !$offline && $canUpdateContactInfo == true}
				<div class="btn-group btn-group-sm">
					{if $treatPrintNoticesAsPhoneNotices}
						...
						<label for="sendEmail" class="btn btn-sm btn-outline-secondary{* {if $profile->notices == 'a' || $profile->notices == 'p'}active{/if}*}"><input type="radio" value="p" id="sendEmail" name="notices" {if $profile->notices == 'a' || $profile->notices == 'p'}checked="checked"{/if}> Telephone</label>
					{else}
						<label for="noticesMail" class="btn btn-sm btn-outline-secondary{* {if $profile->notices == 'a'}active{/if}*}"><input type="radio" value="a" id="noticesMail" name="notices" {if $profile->notices == 'a'}checked="checked"{/if}> Postal Mail</label>
