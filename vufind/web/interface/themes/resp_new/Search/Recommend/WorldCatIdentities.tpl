  ...
  <div class="authorbox">
    ...
    <dl class="narrow_begin">
      {foreach from=$WorldCatIdentities item=subjects name=narrowLoop key=author}
        ...
        {/if}
        <dd>&bull; <a href="/Search/Results?lookfor=%22{$author|escape:"url"}%22&type=Author">{$author|escape}</a>
        {if count($subjects) > 0}
          ...
          {foreach from=$subjects item=subj name=subjLoop}
            ...
            {/if}
            <dd>&bull; <a href="/Search/Results?lookfor=%22{$subj|escape:"url"}%22&type=Subject">{$subj|escape}</a></dd>
          {/foreach}
