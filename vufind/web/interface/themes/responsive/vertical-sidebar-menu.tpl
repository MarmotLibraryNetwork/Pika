{strip}
	{if $displaySidebarMenu}
		<div class="d-sm-none d-md-block col-sm-1 col-md-1 col-lg-1" id="vertical-menu-bar-wrapper">
			<div id="vertical-menu-bar">
				...
				<script>
					...
					{elseif ($module == "Search" && $action != 'History') || $module == "Series" || $module == "Author" || $module == "Genealogy" || $module == "Library"
							...
							|| (($module == 'Archive' || $module == 'Archive2') && ($action == 'Results' || $action == 'RelatedEntities' || ($archiveOnlyInterface && $action == 'Home')))
					    }
						...
						{elseif (!$isLoginPage && !in_array($action, array('EmailResetPin', 'ResetPin', 'SelfReg', 'OfflineCirculation', 'MarcValidations', 'CiteList'))) && ($module == "MyAccount" || $module == "Admin" || $module == "Log" || $module == "Circa" || $module == "LibrarianReview" || $module == "Report" || ($loggedIn && $module == 'Search' && $action == 'History'))}
							{* Prevent this action on the Pin Reset Page && Login Page && Offline Circulation Page*}
							{* On Search/History, only click Account Menu if the user is logged in *}
