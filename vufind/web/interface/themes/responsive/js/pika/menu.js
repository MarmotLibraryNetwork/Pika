...
Pika.Menu = (function(){
	$(function(){
		...
		Pika.Menu.AllSideBarSelectors =
				Pika.Menu.SearchBoxSelectors + ',' +
				Pika.Menu.SideBarSearchSelectors + ',' +
				Pika.Menu.SideBarAccountSelectors + ',' +
				Pika.Menu.SideBarMenuSelectors + ',' +
				Pika.Menu.ExploreMoreSelectors;

		// Set up Sticky Menus
		Pika.Menu.stickyMenu('#horizontal-menu-bar-container', 'sticky-menu-bar');
		Pika.Menu.stickyMenu('#vertical-menu-bar', 'sticky-sidebar');
				...
				//	console.log('Entered SideBar Mode');
					...
					if ($('#vertical-menu-bar').length) { // Sidebar Menu is in use
						...
						if ($('#mobile-menu-search-icon', '#horizontal-menu-bar-container').is('.menu-icon-selected')) {
							...
							Pika.Menu.Mobile.resetRefineSearch();

							Pika.Menu.SideBar.showSearch('.menu-bar-option:nth-child(1)>a')
						}
						else if ($('#mobile-menu-account-icon', '#horizontal-menu-bar-container').is('.menu-icon-selected')) {
							Pika.Menu.SideBar.showAccount('.menu-bar-option:nth-child(2)>a')
						}
						else if ($('#mobile-menu-menu-icon', '#horizontal-menu-bar-container').is('.menu-icon-selected')) {
							Pika.Menu.SideBar.showMenu('.menu-bar-option:nth-child(3)>a')
						}
						else if ($('#mobile-menu-explore-more-icon', '#horizontal-menu-bar-container').is('.menu-icon-selected')) {
							Pika.Menu.SideBar.showExploreMore('.menu-bar-option:nth-child(4)>a')
						} else {
							// if nothing selected, Collapse sidebar
							if ($(Pika.Menu.AllSideBarSelectors).filter(':visible').length == 0) {
						...
						$(Pika.Menu.AllSideBarSelectors).show();
					}
				}
			} else {
				// Entered Mobile Mode
	...
	var reloadRelatedTitles = function() {
		if ($(Pika.Menu.ExploreMoreSelectors).is(':visible')) {
	...
	return {
		...
		hideAllFast: function(){
			return $(Pika.Menu.AllSideBarSelectors).filter(':visible').hide() // return of object is needed for $when(Pika.Menu.hideAll()).done() calls
		},
		...
		SideBar: {
			hideAll: function(){
				return $(Pika.Menu.AllSideBarSelectors).filter(':visible').animate({width:'toggle'},350); // slide left to right
			},
			...
			showSearch: function(clickedElement){
				if ($('#horizontal-search-box').is(':visible')) { // horizontal search box is being used
					this.showMenuSection(Pika.Menu.SideBarSearchSelectors, clickedElement);
				} else { // sidebar search box is being used
					this.showMenuSection('#home-page-search,' + Pika.Menu.SideBarSearchSelectors, clickedElement);
				}
			},

			showMenu: function(clickedElement){
				this.showMenuSection(Pika.Menu.SideBarMenuSelectors, clickedElement)
			},
			...
			showExploreMore: function(clickedElement){
				this.showMenuSection(Pika.Menu.ExploreMoreSelectors, clickedElement, reloadRelatedTitles)
			},
		...
		Mobile: {
			hideAll: function(){
				return $(Pika.Menu.AllSideBarSelectors).filter(':visible').slideUp() // return of object is needed for $.when(Pika.Menu.hideAll()).done() calls
			},

			showMenuSection: function(sectionSelector, clickedElement){
				return $.when(this.hideAll() ).done(function(){
					var elem = $(clickedElement);
						Pika.Menu.openSideBar();  // covers the case when view has switched from sidebar mode to mobile mode
			...
			showExploreMore: function(clickedElement){
				this.showMenuSection(Pika.Menu.ExploreMoreSelectors, clickedElement)
			...
			showSearchFacets: function(){
				...
				if (text == 'Refine Search') {
					$(Pika.Menu.SideBarSearchSelectors).slideDown();
					btn.text('Hide Refine Search');
				}
				if (text == 'Hide Refine Search') {
					$(Pika.Menu.SideBarSearchSelectors).slideUp();
					btn.text('Refine Search');
				}

				//btn.text( text == 'Refine Search' ? 'Hide Refine Search' : 'Refine Search' );
				//$('#refineSearch,'+Pika.Menu.SideBarSearchSelectors).toggle();
			},

			resetRefineSearch: function(){
				$(Pika.Menu.SideBarSearchSelectors).hide();
