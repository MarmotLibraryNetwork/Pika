...
Pika.Responsive=function(){$(function(){$("#lookfor").on("keyup",function(event){$(this).height(0);if(this.scrollHeight<32){$(this).height(18)}else{$(this).height(this.scrollHeight)}}).keyup();$("#lookfor").on("keydown",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();$("#searchForm").submit();return false}}).on("keypress",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();return false}})});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){Pika.Responsive.isPrint=mql.matches})}catch(err){}window.onbeforeprint=function(){Pika.Responsive.isPrint=true};return{originalSidebarHeight:-1,adjustLayout:function(){var resolutionX=document.documentElement.clientWidth;if(resolutionX>=768&&!Pika.Responsive.isPrint){var mainContentElement=$("#main-content-with-sidebar");var sidebarContentElem=$("#sidebar-content");if(Pika.Responsive.originalSidebarHeight==-1){Pika.Responsive.originalSidebarHeight=sidebarContentElem.height()}var heightToTest=sidebarContentElem.height();var maxHeight=Math.max(mainContentElement.height()+15,heightToTest);if(mainContentElement.height()+15!=maxHeight){mainContentElement.height(maxHeight)}if(sidebarContentElem.height()!=maxHeight){sidebarContentElem.height(maxHeight)}}}}}(Pika.Responsive||{});
