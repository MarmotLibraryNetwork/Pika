...
var Globals=function(){return{path:"",url:"",loggedIn:false,opac:false,automaticTimeoutLength:0,automaticTimeoutLengthLoggedOut:0,encodedRepositoryUrl:"",debugViewer:false,activeAction:"",activeModule:""}}(Globals||{});
...
Pika.Browse=function(){return{curPage:1,loadingMoreResults:false,curCategory:"",curSubCategory:"",browseMode:"covers",browseModeClasses:{covers:"home-page-browse-thumbnails",grid:"home-page-browse-grid"},addToHomePage:function(searchId,searchSource){searchSource=searchSource||"catalog";Pika.Account.ajaxLightbox("/Browse/AJAX?method=getAddBrowseCategoryForm&searchId="+searchId+"&searchSource="+encodeURIComponent(searchSource),true);return false},initializeBrowseCategory:function(){var browseCategoryCarousel=$("#browse-category-carousel");browseCategoryCarousel.on("jcarousel:targetin","li",function(){var categoryId=$(this).data("category-id");Pika.Browse.changeBrowseCategory(categoryId)});if($("#browse-category-picker .jcarousel-control-prev").css("display")!="none"){browseCategoryCarousel.find("li").on("click",function(n){$("#browse-category-carousel").jcarousel("scroll",$(this))});var scrollFactor=15;browseCategoryCarousel.touchwipe({wipeLeft:function(dx){var scrollInterval=Math.round(dx/scrollFactor);$("#browse-category-carousel").jcarousel("scroll","+="+scrollInterval)},wipeRight:function(dx){var scrollInterval=Math.round(dx/scrollFactor);$("#browse-category-carousel").jcarousel("scroll","-="+scrollInterval)}})}else{browseCategoryCarousel.find("li").click(function(){$(this).trigger("jcarousel:targetin")})}},setTabs:function(){setTimeout(function(){if($("#browse-category-carousel").css("overflow")!="visible"){$(".jcarousel").jcarousel("items").children("button").attr("tabindex","-1");var visible=$(".jcarousel").jcarousel("fullyvisible");visible.children("button").attr("tabindex","0")}},100)},toggleBrowseMode:function(selectedMode){var categoryTextId=this.curCategory||$("#browse-category-carousel .selected").data("category-id"),subCategoryTextId=this.curSubCategory||$("#browse-sub-category-menu .selected").data("sub-category-id");this.browseMode=this.browseModeClasses.hasOwnProperty(selectedMode)?selectedMode:this.browseMode;$(".browseMode").removeClass("active");$("#"+this.browseMode).addClass("active");if(!Globals.opac&&Pika.hasLocalStorage()){window.localStorage.setItem("browseMode",this.browseMode)}if(subCategoryTextId)return this.changeBrowseSubCategory(subCategoryTextId);else return this.changeBrowseCategory(categoryTextId)},resetBrowseResults:function(){var classes=function(){var str="",object=Pika.Browse.browseModeClasses;for(property in object){str+=object[property]+" "}return str}(),selectedClass=this.browseModeClasses[this.browseMode];$("#home-page-browse-results").children().fadeOut(function(){$("#home-page-browse-results").children().slice(1).remove();$("#home-page-browse-results div.row").removeClass(classes).addClass(selectedClass)})},changeBrowseCategory:function(categoryTextId){var url="/Browse/AJAX",params={method:"getBrowseCategoryInfo",textId:categoryTextId||Pika.Browse.curCategory,browseMode:this.browseMode},newLabel=$("#browse-category-"+categoryTextId+" button").first().text(),loadingID=categoryTextId||initial;if(Pika.Browse.initialSubCategory!==""){params.subCategoryTextId=Pika.Browse.initialSubCategory;delete Pika.Browse.initialSubCategory}$(".browse-category").removeClass("selected");$("#browse-category-"+categoryTextId).addClass("selected");$("#selected-browse-search-link").attr("href","#");$(".selected-browse-label-search-text,.selected-browse-sub-category-label-search-text").fadeOut(function(){$(".selected-browse-label-search-text").html(newLabel).fadeIn()});$("#browse-sub-category-menu").children().fadeOut(function(){$(this).remove()});this.resetBrowseResults();this.loadingCategory=loadingID;Pika.Browse.setTabs();$.getJSON(url,params,function(data){if(Pika.Browse.loadingCategory==loadingID){if(data.success==false){Pika.showMessage("Error loading browse information","Sorry, we were not able to find titles for that category")}else{$(".selected-browse-label-search-text").html(data.label);Pika.Browse.curPage=1;$("#more-browse-results").show();$("#more-results-status").text("");Pika.Browse.curCategory=data.textId;Pika.Browse.curSubCategory=data.subCategoryTextId||"";$("#home-page-browse-results div.row").html(data.records).fadeIn("slow");$("#selected-browse-search-link").attr("href",data.searchUrl);if(data.subcategories){$("#browse-sub-category-menu").html(data.subcategories).fadeIn();if(data.subCategoryTextId){$(".selected-browse-sub-category-label-search-text").html($("#browse-sub-category-"+data.subCategoryTextId).addClass("selected").text()).fadeIn()}}}}}).fail(function(){Pika.ajaxFail();$("#home-page-browse-results div").html("").show()}).done(function(){$("#more-browse-results").removeAttr("style");Pika.Browse.loadingCategory=null});return false},changeBrowseSubCategory:function(subCategoryTextId){var url="/Browse/AJAX",params={method:"getBrowseSubCategoryInfo",textId:Pika.Browse.curCategory,subCategoryTextId:subCategoryTextId,browseMode:this.browseMode};$("#browse-sub-category-menu button").removeClass("selected");newSubCategoryLabel=$("#browse-sub-category-"+subCategoryTextId).addClass("selected").text();$(".selected-browse-sub-category-label-search-text").fadeOut(function(){$(this).html(newSubCategoryLabel).fadeIn()});this.resetBrowseResults();$.getJSON(url,params,function(data){if(data.success==false){Pika.showMessage("Error loading browse information","Sorry, we were not able to find titles for that category")}else{if(data.label)$(".selected-browse-label-search-text").html(data.label);if(data.subCategoryLabel)$(".selected-browse-sub-category-label-search-text").html(data.subCategoryLabel);else $(".selected-browse-sub-category-label-search-text").fadeOut();Pika.Browse.curPage=1;$("#more-browse-results").show();$("#more-results-status").text("");if(data.textId)Pika.Browse.curCategory=data.textId;if(data.subCategoryTextId)Pika.Browse.curSubCategory=data.subCategoryTextId||"";$("#home-page-browse-results div.row").html(data.records).fadeIn("slow");$("#selected-browse-search-link").attr("href",data.searchUrl)}}).fail(function(){Pika.ajaxFail();$("#home-page-browse-results div.row").html("").show();$(".selected-browse-sub-category-label-search-text").fadeOut()}).done(function(){$("#more-browse-results").removeAttr("style")});return false},createBrowseCategory:function(){var url="/Browse/AJAX",searchSource=$("#searchSource").val()||"catalog",params={method:"createBrowseCategory",categoryName:$("#categoryName").val(),searchSource:searchSource,addAsSubCategoryOf:$("#addAsSubCategoryOfSelect").val()};var searchId=$("#searchId");if(searchId){params["searchId"]=searchId.val()}var listId=$("#listId");if(listId){params["listId"]=listId.val()}$.getJSON(url,params,function(data){if(data.success==false){Pika.showMessage("Unable to create category",data.message)}else{Pika.showMessageWithButtons("Successfully added",data.message,data.buttons,true)}}).fail(Pika.ajaxFail);return false},getMoreResults:function(){if(this.loadingMoreResults){return false}var url="/Browse/AJAX",params={method:"getMoreBrowseResults",textId:this.curSubCategory||this.curCategory,pageToLoad:this.curPage+1,browseMode:this.browseMode},status=$("#more-results-status"),loading=$("#more-results-loading"),button=$("#more-browse-results"),divClass=this.browseModeClasses[this.browseMode];this.loadingMoreResults=true;button.prop("disabled",true);loading.removeClass("d-none");status.text("Loading more results.");$.getJSON(url,params,function(data){if(data.success==false){status.text("");Pika.showMessage("Error loading browse information","Sorry, we were not able to find titles for that category")}else{var newDiv=$('<div class="'+divClass+' row" />').hide().append(data.records);$("."+divClass).filter(":last").after(newDiv).after("<hr>");newDiv.fadeIn("slow");if(data.lastPage){button.hide();status.text("Loaded the last of the results.")}else{Pika.Browse.curPage++;status.text("More results loaded.")}}}).fail(function(){status.text("");Pika.ajaxFail.apply(this,arguments)}).always(function(){Pika.Browse.loadingMoreResults=false;loading.addClass("d-none");button.prop("disabled",false)});return false}}}(Pika.Browse||{});
...
Pika.Account.ReadingHistory=function(){return{deletedMarkedAction:function(){Pika.confirm('<p class="alert alert-warning">The marked items will be irreversibly deleted from your reading history.  Proceed?</p>',function(){$("#readingHistoryAction").val("deleteMarked");$("#readingListForm").submit()});return false},deleteAllAction:function(){Pika.confirm('<p class="alert alert-danger">Your entire reading history will be irreversibly deleted.  Proceed?</p>',function(){$("#readingHistoryAction").val("deleteAll");$("#readingListForm").submit()});return false},optOutAction:function(){Pika.confirm('<p class="alert alert-danger">Opting out of Reading History will also <strong>delete your entire reading history</strong> irreversibly.  Proceed?</p>',function(){$("#readingHistoryAction").val("optOut");$("#readingListForm").submit()});return false},optInAction:function(){$("#readingHistoryAction").val("optIn");$("#readingListForm").submit();return false},exportListAction:function(){$("#readingHistoryAction").val("exportToExcel");$("#readingListForm").submit();return false},searchReadingHistoryAction:function(){$("#readingHistoryAction").val("searchReadingHistory");$("#readingListForm").submit();return false}}}(Pika.Account.ReadingHistory||{});
...
Pika.Responsive=function(){$(function(){$("#lookfor").on("keydown",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();$("#searchForm").submit();return false}}).on("keypress",function(event){if(event.which==13||event.which==10){event.preventDefault();event.stopPropagation();return false}})});try{var mediaQueryList=window.matchMedia("print");mediaQueryList.addListener(function(mql){Pika.Responsive.isPrint=mql.matches})}catch(err){}window.onbeforeprint=function(){Pika.Responsive.isPrint=true};return{}}(Pika.Responsive||{});
...
function TitleScroller(scrollerId,scrollerShortName,container,autoScroll,style,tabSelect){this.scrollerTitles=[];this.currentScrollerIndex=0;this.numScrollerTitles=0;this.scrollerId=scrollerId;this.scrollerShortName=scrollerShortName;this.container=container;this.scrollInterval=0;this.swipeInterval=5;this.autoScroll=typeof autoScroll=="undefined"?false:autoScroll;this.style=typeof style=="undefined"?"horizontal":style;this.tabSelect=typeof tabSelect=="undefined"?false:tabSelect}TitleScroller.prototype.loadTitlesFrom=function(jsonUrl){jsonUrl=decodeURIComponent(jsonUrl);var scroller=this,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");scrollerBody.hide();$("#titleScrollerSelectedTitle"+this.scrollerShortName+",#titleScrollerSelectedAuthor"+this.scrollerShortName).html("");$(".scrollerLoadingContainer").show();$.getJSON(jsonUrl,function(data){scroller.loadTitlesFromJsonData(data)}).fail(function(){scrollerBody.html("Unable to load titles. Please try again later.").show();$(".scrollerLoadingContainer").hide()})};TitleScroller.prototype.loadTitlesFromJsonData=function(data){var scroller=this,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");try{if(data.error)throw{description:data.error};if(data.titles.length==0){scrollerBody.html("No titles were found for this list. Please try again later.");$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerLoadingContainer").hide();scrollerBody.show()}else{scroller.scrollerTitles=[];var i=0;$.each(data.titles,function(key,val){scroller.scrollerTitles[i++]=val});if(scroller.container&&data.titles.length>0){$("#"+scroller.container).fadeIn()}scroller.numScrollerTitles=data.titles.length;if(this.style=="horizontal"||this.style=="vertical"){scroller.currentScrollerIndex=data.currentIndex}else{scroller.currentScrollerIndex=0}TitleScroller.prototype.updateScroller.call(scroller)}}catch(err){if(scrollerBody!=null){scrollerBody.html("<div class='alert alert-warning'>Error loading titles from data : '"+err.description+"' Please try again later.</div>").show();$(".scrollerLoadingContainer").hide()}}};TitleScroller.prototype.loadIssuesFromAjax=function(jsonUrl){var scroller=this,scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");jsonUrl=decodeURIComponent(jsonUrl);scrollerBody.hide();$("#titleScrollerSelectedTitle"+this.scrollerShortName+",#titleScrollerSelectedAuthor"+this.scrollerShortName).html("");$(".scrollerLoadingContainer").show();$.getJSON(jsonUrl,function(data){if(data.error)throw{description:data.error};if(data.length==0){ScrollerBody.html("No issues were found for this magazine. Please try again later");$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerLoadingContainer").hide();scrollerBody.show()}else{scroller.scrollerTitles=[];var i=0;$.each(data,function(key,val){scroller.scrollerTitles[i++]=val});if(scroller.container&&data.length>0){$("#"+scroller.container).fadeIn()}scroller.numScrollerTitles=data.length;if(this.style=="horizontal"||this.style=="vertical"){scroller.currentScrollerIndex=data.currentIndex}else{scroller.currentScrollerIndex=0}TitleScroller.prototype.updateScroller.call(scroller)}}).fail(function(er){})};TitleScroller.prototype.updateScroller=function(){var scrollerBody=$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody");try{var scrollerBodyContents="",curScroller=this;if(this.style=="horizontal"){for(var i in this.scrollerTitles){scrollerBodyContents+=this.scrollerTitles[i]["formattedTitle"]}scrollerBody.html(scrollerBodyContents).width(this.scrollerTitles.length*300).waitForImages(function(){TitleScroller.prototype.finishLoadingScroller.call(curScroller)})}else if(this.style=="vertical"){for(var j in this.scrollerTitles){scrollerBodyContents+=this.scrollerTitles[j]["formattedTitle"]}scrollerBody.html(scrollerBodyContents).height(this.scrollerTitles.length*131).waitForImages(function(){TitleScroller.prototype.finishLoadingScroller.call(curScroller)})}else if(this.style=="text-list"){for(var i in this.scrollerTitles){scrollerBodyContents+=this.scrollerTitles[i]["formattedTextOnlyTitle"]}scrollerBody.html(scrollerBodyContents).height(this.scrollerTitles.length*40);TitleScroller.prototype.finishLoadingScroller.call(curScroller)}else{this.currentScrollerIndex=0;scrollerBody.html(this.scrollerTitles[this.currentScrollerIndex]["formattedTitle"]);TitleScroller.prototype.finishLoadingScroller.call(this)}}catch(err){scrollerBody.html("<div class='alert alert-warning'>Error loading titles from data: '"+err+"' Please try again later.</div>").show();$(".scrollerLoadingContainer").hide()}};TitleScroller.prototype.finishLoadingScroller=function(){$(".scrollerLoadingContainer").hide();$("#"+this.scrollerId+" .scrollerBodyContainer .scrollerBody").show();TitleScroller.prototype.activateCurrentTitle.call(this);var curScroller=this;$(".scrollerTitle").on("mouseover",null,{scroller:curScroller},function(){curScroller.hovered=true}).on("mouseout",null,{scroller:curScroller},function(){curScroller.hovered=false});curScroller.hovered=false;curScroller.interval=curScroller.interval==5e3||typeof curScroller.interval=="undefined"?5e3:curScroller.interval;curScroller.updateInterval=curScroller.updateInterval==false||typeof curScroller.updateInterval=="undefined"?false:curScroller.updateInterval;if(this.autoScroll&&this.scrollInterval==0){curScroller.scrolling=true;this.scrollInterval=setInterval(function(){if(!curScroller.hovered&&curScroller.scrolling===true){curScroller.scrollToRight()}},5e3)}if(this.tabSelect){$("button.scrollerTitle").on("focus",function(){let scrollerTitleId="scrollerTitle"+curScroller.scrollerShortName;let scrollerIndex=$(this).attr("id").replace(scrollerTitleId,"");let index=scrollerIndex.replace(".scrollerTitle","");curScroller.scrollToIndex(index)})}};TitleScroller.prototype.playPauseControl=function(sender,scroller){let scrolling=scroller.scrolling;let controlWrapper=$("#"+scroller.scrollerId).parent(".titleScrollerWrapper").children(".sliderControls");if(scrolling==true){scroller.scrolling=false;controlWrapper.children("button.pause").addClass("play bi-play-fill");controlWrapper.children("button.play").removeClass("pause bi-pause-fill");controlWrapper.children("button.play").children("span").html("Resume");controlWrapper.children("button.play").attr("aria-label","Resume")}else{scroller.scrolling=true;controlWrapper.children("button.play").addClass("pause bi-pause-fill");controlWrapper.children("button.pause").removeClass("play bi-play-fill");controlWrapper.children("button.pause").children("span").html("Pause");controlWrapper.children("button.pause").attr("aria-label","Pause")}};TitleScroller.prototype.fasterControl=function(scroller){let interval=scroller.interval;let controlWrapper=$("#"+scroller.scrollerId).parent(".titleScrollerWrapper").children(".sliderControls");controlWrapper.children("button.slowDown").prop("disabled",false);controlWrapper.children("button.speedUp").attr("aria-label","Speed Up");let newInterval=interval-500;if(newInterval<1e3){$(controlWrapper).children("button.speedUp").prop("disabled",true);interval=500}else{interval=newInterval}scroller.interval=interval;clearInterval(scroller.scrollInterval);clearInterval(this.scrollInterval);scroller.scrollInterval=setInterval(function(){if(!scroller.hovered&&scroller.scrolling===true){scroller.scrollToRight()}},interval);let intervalHuman=interval/1e3;controlWrapper.children("button.speedUp").attr("aria-label","Speed: "+intervalHuman)};TitleScroller.prototype.slowerControl=function(scroller){let interval=scroller.interval;let controlWrapper=$("#"+scroller.scrollerId).parent(".titleScrollerWrapper").children(".sliderControls");controlWrapper.children("button.speedUp").prop("disabled",false);controlWrapper.children("button.slowDown").attr("aria-label","Slow Down");let newInterval=interval+500;if(newInterval>1e4){$(controlWrapper).children("button.slowDown").prop("disabled",true);interval=10500}else{interval=newInterval}scroller.interval=interval;clearInterval(scroller.scrollInterval);clearInterval(this.scrollInterval);scroller.scrollInterval=setInterval(function(){if(!scroller.hovered&&scroller.scrolling===true){scroller.scrollToRight()}},interval);let intervalHuman=interval/1e3;controlWrapper.children("button.slowDown").attr("aria-label","Speed: "+intervalHuman)};TitleScroller.prototype.setVisibleTabIndex=function(scroller){var scrollerId=scroller.scrollerId,width=$("#"+scrollerId).width(),coverWidth=$("#scrollerTitle"+scroller.scrollerShortName+scroller.currentScrollerIndex).width(),displayedItems=Math.floor(width/(coverWidth+30)),currentIndex=scroller.currentScrollerIndex;let indexArray=[];$("#"+scrollerId+" button.scrollerTitle").attr("tabindex","-1");if(currentIndex+displayedItems>scroller.numScrollerTitles){let overflowTitles=currentIndex+displayedItems-scroller.numScrollerTitles;for(let n=currentIndex;n<currentIndex+displayedItems-overflowTitles;n++){indexArray.push(n)}}else{for(let n=currentIndex;n<currentIndex+displayedItems;n++){indexArray.push(n)}}indexArray.forEach(function(i){$("#scrollerTitle"+scroller.scrollerShortName+i).attr("tabindex","0")})};TitleScroller.prototype.scrollToRight=function(){this.currentScrollerIndex++;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.scrollToLeft=function(){this.currentScrollerIndex--;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeToRight=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex-=customSwipeInterval;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeToLeft=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex+=customSwipeInterval;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeUp=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex-=customSwipeInterval;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.swipeDown=function(customSwipeInterval){customSwipeInterval=typeof customSwipeInterval==="undefined"?this.swipeInterval:customSwipeInterval;this.currentScrollerIndex+=customSwipeInterval;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0;TitleScroller.prototype.activateCurrentTitle.call(this)};TitleScroller.prototype.scrollToIndex=function(index){var previousIndex=typeof this.currentScrollerIndex=="undefined"?0:this.currentScrollerIndex;if(index>previousIndex){this.currentScrollerIndex++;if(this.currentScrollerIndex>this.numScrollerTitles-1)this.currentScrollerIndex=0}else if(index<previousIndex){this.currentScrollerIndex--;if(this.currentScrollerIndex<0)this.currentScrollerIndex=this.numScrollerTitles-1}else{this.currentScrollerIndex=this.currentScrollerIndex}TitleScroller.prototype.activateCurrentTitle(false,this)};TitleScroller.prototype.activateCurrentTitle=function(scroll,curScroller){var scroller=typeof curScroller=="undefined"?this:curScroller;if(scroller.numScrollerTitles===0){return}var scrollerTitles=scroller.scrollerTitles,scrollerShortName=scroller.scrollerShortName,currentScrollerIndex=scroller.currentScrollerIndex,scrollerBody=$("#"+scroller.scrollerId+" .scrollerBodyContainer .scrollerBody"),scrollerTitleId="#scrollerTitle"+scroller.scrollerShortName+currentScrollerIndex;$("#tooltip").hide();if(scroller.style=="horizontal"){$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);if($(scrollerTitleId).length!=0){var widthItemsLeft=$(scrollerTitleId).position().left,widthCurrent=$(scrollerTitleId).width(),containerWidth=$("#"+scroller.scrollerId+" .scrollerBodyContainer").width();if(scroller.tabSelect){var leftPosition=-widthItemsLeft;var doScroll=typeof scroll=="undefined"?true:scroll;if(doScroll){scrollerBody.animate({left:leftPosition+"px"},200,function(){for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")});TitleScroller.prototype.setVisibleTabIndex(scroller)}else{for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")}}else{var leftPosition=-(widthItemsLeft+widthCurrent/2-containerWidth/2);scrollerBody.animate({left:leftPosition+"px"},400,function(){for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")})}}}else if(scroller.style=="vertical"){$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);if($(scrollerTitleId).length!=0){var relativeTopOfElement=$(scrollerTitleId).position().top,topPosition=25-relativeTopOfElement;scrollerBody.animate({top:topPosition+"px"},400,function(){for(var i in scrollerTitles){var scrollerTitleId2="#scrollerTitle"+scrollerShortName+i;$(scrollerTitleId2).removeClass("selected")}$(scrollerTitleId).addClass("selected")})}}else if(scroller.style=="text-list"){}else{$("#titleScrollerSelectedTitle"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["title"]);$("#titleScrollerSelectedAuthor"+scrollerShortName).html(scrollerTitles[currentScrollerIndex]["author"]);scrollerBody.left="0px";scrollerBody.html(scroller.scrollerTitles[currentScrollerIndex]["formattedTitle"])}};(function($){$.fn.waitForImages=function(finishedCallback,eachCallback){eachCallback=eachCallback||function(){};if(!$.isFunction(finishedCallback)||!$.isFunction(eachCallback)){throw{name:"invalid_callback",message:"An invalid callback was supplied."}}var objs=$(this),allImgs=objs.find("img"),allImgsLength=allImgs.length,allImgsLoaded=0;if(allImgsLength==0){finishedCallback.call(this)}else{setTimeout(function(){finishedCallback.call(this)},1e4)}return objs.each(function(){var obj=$(this),imgs=obj.find("img");if(imgs.length==0){return}imgs.each(function(){var image=new Image,imgElement=this;image.onload=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.onabort=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.onerror=function(){allImgsLoaded++;eachCallback.call(imgElement,allImgsLoaded,allImgsLength);if(allImgsLoaded==allImgsLength){finishedCallback.call(obj[0])}return false};image.src=this.src})})}})(jQuery);
