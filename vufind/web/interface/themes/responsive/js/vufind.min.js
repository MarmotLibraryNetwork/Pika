...
VuFind.GroupedWork=function(){return{hasTableOfContentsInRecord:false,clearUserRating:function(groupedWorkId){var url="/GroupedWork/"+groupedWorkId+"/AJAX?method=clearUserRating";$.getJSON(url,function(data){if(data.result==true){$(".rate"+groupedWorkId).find(".ui-rater-starsOn").width(0);$("#myRating"+groupedWorkId).hide();VuFind.showMessage("Success",data.message,true)}else{VuFind.showMessage("Sorry",data.message)}});return false},clearNotInterested:function(notInterestedId){var url="/GroupedWork/"+notInterestedId+"/AJAX?method=clearNotInterested";$.getJSON(url,function(data){if(data.result==false){VuFind.showMessage("Sorry","There was an error updating the title.")}else{$("#notInterested"+notInterestedId).hide()}})},deleteReview:function(id,reviewId){VuFind.confirm("Are you sure you want to delete this review?",function(){var url="/GroupedWork/"+id+"/AJAX?method=deleteUserReview";$.getJSON(url,function(data){if(data.result==true){$("#review_"+reviewId).hide();VuFind.showMessage("Success",data.message,true)}else{VuFind.showMessage("Sorry",data.message)}})});return false},getGoDeeperData:function(id,dataType){var placeholder;if(dataType=="excerpt"){placeholder=$("#excerptPlaceholder")}else if(dataType=="avSummary"){placeholder=$("#avSummaryPlaceholder")}else if(dataType=="tableOfContents"){placeholder=$("#tableOfContentsPlaceholder")}else if(dataType=="authornotes"){placeholder=$("#authornotesPlaceholder")}if(placeholder.hasClass("loaded"))return;placeholder.show();var url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"GetGoDeeperData",dataType:dataType};$.getJSON(url,params,function(data){placeholder.html(data.formattedData).addClass("loaded")})},getGoodReadsComments:function(isbn){$("#goodReadsPlaceHolder").replaceWith("<iframe id='goodreads_iframe' class='goodReadsIFrame' src='https://www.goodreads.com/api/reviews_widget_iframe?did=DEVELOPER_ID&format=html&isbn="+isbn+"&links=660&review_back=fff&stars=000&text=000' width='100%' height='400px' frameborder='0'></iframe>")},reloadEnrichment:function(id){VuFind.GroupedWork.loadEnrichmentInfo(id,true)},loadEnrichmentInfo:function(id,forceReload){var url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"getEnrichmentInfo"};if(forceReload!==undefined){params["reload"]=true}$.getJSON(url,params,function(data){try{var seriesData=data.seriesInfo;if(seriesData&&seriesData.titles.length>0){seriesScroller=new TitleScroller("titleScrollerSeries","Series","seriesList");$("#seriesInfo").show();seriesScroller.loadTitlesFromJsonData(seriesData);$("#seriesPanel").show()}else{$("#seriesPanel").hide()}var similarTitleData=data.similarTitles;if(similarTitleData&&similarTitleData.titles.length>0){morelikethisScroller=new TitleScroller("titleScrollerMoreLikeThis","MoreLikeThis","morelikethisList");$("#moreLikeThisInfo").show();morelikethisScroller.loadTitlesFromJsonData(similarTitleData)}var showGoDeeperData=data.showGoDeeper;if(showGoDeeperData){var goDeeperOptions=data.goDeeperOptions;for(var option in goDeeperOptions){if(option=="excerpt"){$("#excerptPanel").show()}else if(option=="avSummary"){$("#avSummaryPlaceholder,#tableOfContentsPlaceholder,#tableOfContentsPanel").show()}else if(option=="tableOfContents"){$("#tableOfContentsPlaceholder,#tableOfContentsPanel").show()}else if(option=="authorNotes"){$("#authornotesPlaceholder,#authornotesPanel").show()}}}if(VuFind.GroupedWork.hasTableOfContentsInRecord){$("#tableofcontentstab_label,#tableOfContentsPlaceholder,#tableOfContentsPanel").show()}var relatedContentData=data.relatedContent;if(relatedContentData&&relatedContentData.length>0){$("#relatedContentPlaceholder").html(relatedContentData)}var similarTitlesNovelist=data.similarTitlesNovelist;if(similarTitlesNovelist&&similarTitlesNovelist.length>0){$("#novelisttitlesPlaceholder").html(similarTitlesNovelist);$("#novelisttab_label,#similarTitlesPanel").show()}var similarAuthorsNovelist=data.similarAuthorsNovelist;if(similarAuthorsNovelist&&similarAuthorsNovelist.length>0){$("#novelistauthorsPlaceholder").html(similarAuthorsNovelist);$("#novelisttab_label,#similarAuthorsPanel").show()}var similarSeriesNovelist=data.similarSeriesNovelist;if(similarSeriesNovelist&&similarSeriesNovelist.length>0){$("#novelistseriesPlaceholder").html(similarSeriesNovelist);$("#novelisttab_label,#similarSeriesPanel").show()}$(".ajax-carousel","#explore-more-body").parents(".jcarousel-wrapper").show().prev(".sectionHeader").show();VuFind.initCarousels(".ajax-carousel")}catch(e){alert("error loading enrichment: "+e)}})},loadReviewInfo:function(id){var url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX?method=getReviewInfo";$.getJSON(url,function(data){if(data.numSyndicatedReviews==0){$("#syndicatedReviewsPanel").hide()}else{var syndicatedReviewsData=data.syndicatedReviewsHtml;if(syndicatedReviewsData&&syndicatedReviewsData.length>0){$("#syndicatedReviewPlaceholder").html(syndicatedReviewsData)}}if(data.numEditorialReviews==0){$("#editorialReviewsPanel").hide()}else{var editorialReviewsData=data.editorialReviewsHtml;if(editorialReviewsData&&editorialReviewsData.length>0){$("#editorialReviewPlaceholder").html(editorialReviewsData)}}if(data.numCustomerReviews==0){$("#borrowerReviewsPanel").hide()}else{var customerReviewsData=data.customerReviewsHtml;if(customerReviewsData&&customerReviewsData.length>0){$("#customerReviewPlaceholder").html(customerReviewsData)}}})},markNotInterested:function(recordId){VuFind.Account.ajaxLogin(function(){var url="/GroupedWork/"+recordId+"/AJAX?method=markNotInterested";$.getJSON(url,function(data){if(data.result==true){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Sorry",data.message)}});return false})},removeTag:function(id,tag){VuFind.confirm('Are you sure you want to remove the tag "'+tag+'" from this title?',function(){var url="/GroupedWork/"+id+"/AJAX",params={method:"removeTag",tag:tag};$.getJSON(url,params,function(data){if(data.result==true){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Sorry",data.message)}});return false});return false},saveReview:function(id){VuFind.Account.ajaxLogin(function(){var comment=$("#comment"+id).val(),rating=$("#rating"+id).val(),url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"saveReview",comment:comment,rating:rating};$.getJSON(url,params,function(data){if(data.success){if(data.newReview){$("#customerReviewPlaceholder").append(data.reviewHtml)}else{$("#review_"+data.reviewId).replaceWith(data.reviewHtml)}VuFind.closeLightbox()}else{VuFind.showMessage("Error",data.message)}}).fail(VuFind.ajaxFail)});return false},saveTag:function(id){var tag=$("#tags_to_apply").val(),url="/GroupedWork/"+id+"/AJAX",params={method:"saveTag",tag:tag};$("#saveToList-button").prop("disabled",true);$.getJSON(url,params,function(data){if(data.success){VuFind.showMessage("Success",data.message,1)}else{VuFind.showMessage("Error adding tags","There was an unexpected error adding tags to this title.<br>"+data.message)}}).fail(VuFind.ajaxFail);return false},saveToList:function(id){VuFind.Account.ajaxLogin(function(){var listId=$("#addToList-list").val(),notes=$("#addToList-notes").val(),url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"saveToList",notes:notes,listId:listId};$.getJSON(url,params,function(data){if(data.success){VuFind.showMessageWithButtons("Added Successfully",data.message,data.buttons)}else{VuFind.showMessage("Error",data.message)}})});return false},sendEmail:function(id){VuFind.Account.ajaxLogin(function(){var from=$("#from").val(),to=$("#to").val(),message=$("#message").val(),related_record=$("#related_record").val(),url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"sendEmail",from:from,to:to,message:message,related_record:related_record};$.getJSON(url,params,function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}}).fail(VuFind.ajaxFail)});return false},sendSMS:function(id){VuFind.Account.ajaxLogin(function(){var phoneNumber=$("#sms_phone_number").val(),provider=$("#provider").val(),related_record=$("#related_record").val(),url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"sendSMS",provider:provider,sms_phone_number:phoneNumber,related_record:related_record};$.getJSON(url,params,function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}}).fail(VuFind.ajaxFail)});return false},showGroupedWorkInfo:function(id,browseCategoryId){var url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX?method=getWorkInfo";if(browseCategoryId!==undefined){url+="&browseCategoryId="+browseCategoryId}VuFind.loadingMessage();$.getJSON(url,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(VuFind.ajaxFail);return false},forceRegrouping:function(id){return this.basicShowMessageReloadOnSuccess("forceRegrouping",id)},forceReindex:function(id){return this.basicShowMessageReloadOnSuccess("forceReindex",id)},reloadCover:function(id){return this.basicShowMessageReloadOnSuccess("reloadCover",id)},reloadIslandora:function(id){return this.basicShowMessageReloadOnSuccess("reloadIslandora",id)},basicShowMessageReloadOnSuccess:function(method,id){$.getJSON("/GroupedWork/"+encodeURIComponent(id)+"/AJAX?method="+method,function(data){if(data.success){VuFind.showMessage("Success",data.message,true,true)}else{VuFind.showMessage("Error",data.message)}});return false},showEmailForm:function(trigger,id){return this.basicAjaxHandler("getEmailForm",id,trigger)},showReviewForm:function(trigger,id){return this.basicAjaxHandler("getReviewForm",id,trigger)},showSaveToListForm:function(trigger,id){return this.basicAjaxHandler("getSaveToListForm",id,trigger)},showSmsForm:function(trigger,id){return this.basicAjaxHandler("getSMSForm",id,trigger)},showTagForm:function(trigger,id){return this.basicAjaxHandler("getAddTagForm",id,trigger)},basicAjaxHandler:function(method,id,trigger){VuFind.Account.ajaxLogin(function(){VuFind.loadingMessage();$.getJSON("/GroupedWork/"+encodeURIComponent(id)+"/AJAX?method="+method,function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)}).fail(VuFind.ajaxFail)},$(trigger));return false}}}(VuFind.GroupedWork||{});
VuFind.Lists=function(){return{addToHomePage:function(listId){VuFind.Account.ajaxLightbox("/MyAccount/AJAX?method=getAddBrowseCategoryFromListForm&listId="+listId,true);return false},editListAction:function(){$("#listDescription,#listTitle,#FavEdit").hide();$("#listEditControls,#FavSave").show();return false},submitListForm:function(action){$("#myListActionHead").val(action);$("#myListFormHead").submit();return false},makeListPublicAction:function(){return this.submitListForm("makePublic")},makeListPrivateAction:function(){return this.submitListForm("makePrivate")},deleteListAction:function(){if(confirm("Are you sure you want to delete this list?")){this.submitListForm("deleteList")}return false},updateListAction:function(){return this.submitListForm("saveList")},deleteAllListItemsAction:function(){if(confirm("Are you sure you want to delete all titles from this list?  This cannot be undone.")){this.submitListForm("deleteAll")}return false},emailListAction:function(listId){var urlToDisplay="/MyAccount/AJAX";VuFind.loadingMessage();$.getJSON(urlToDisplay,{method:"getEmailMyListForm",listId:listId},function(data){VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons)});return false},SendMyListEmail:function(){var url="/MyAccount/AJAX";$.getJSON(url,{listId:$('#emailListForm input[name="listId"]').val(),to:$('#emailListForm input[name="to"]').val(),from:$('#emailListForm input[name="from"]').val(),message:$('#emailListForm textarea[name="message"]').val(),method:"sendMyListEmail"},function(data){if(data.result){VuFind.showMessage("Success",data.message)}else{VuFind.showMessage("Error",data.message)}})},citeListAction:function(id){return VuFind.Account.ajaxLightbox("/MyAccount/AJAX?method=getCitationFormatsForm&listId="+id,false)},processCiteListForm:function(){$("#citeListForm").submit()},batchAddToListAction:function(id){return VuFind.Account.ajaxLightbox("/MyAccount/AJAX/?method=getBulkAddToListForm&listId="+id)},processBulkAddForm:function(){$("#bulkAddToList").submit()},changeList:function(){var availableLists=$("#availableLists");window.location="/MyAccount/MyList/"+availableLists.val()},printListAction:function(){window.print();return false},importListsFromClassic:function(){if(confirm("This will import any lists you had defined in the old catalog.  This may take several minutes depending on the size of your lists. Are you sure you want to continue?")){window.location="/MyAccount/ImportListsFromClassic"}return false}}}(VuFind.Lists||{});
...
VuFind.Hoopla=function(){return{checkOutHooplaTitle:function(hooplaId,patronId){VuFind.Account.ajaxLogin(function(){if(typeof patronId==="undefined"){patronId=$("#patronId","#pickupLocationOptions").val()}var url="/Hoopla/"+hooplaId+"/AJAX",params={method:"checkOutHooplaTitle",patronId:patronId};if($("#stopHooplaConfirmation").prop("checked")){params["stopHooplaConfirmation"]=true}$.getJSON(url,params,function(data){if(data.success){VuFind.showMessageWithButtons(data.title,data.message,data.buttons)}else{VuFind.showMessage("Checking Out Title",data.message)}}).fail(VuFind.ajaxFail)});return false},getHooplaCheckOutPrompt:function(hooplaId){VuFind.Account.ajaxLogin(function(){var url="/Hoopla/"+hooplaId+"/AJAX?method=getHooplaCheckOutPrompt";$.getJSON(url,function(data){VuFind.showMessageWithButtons(data.title,data.body,data.buttons)}).fail(VuFind.ajaxFail)});return false},returnHooplaTitle:function(patronId,hooplaId){VuFind.confirm("Are you sure you want to return this title?",function(){VuFind.Account.ajaxLogin(function(){VuFind.showMessage("Returning Title","Returning your title in Hoopla.");var url="/Hoopla/"+hooplaId+"/AJAX",params={method:"returnHooplaTitle",patronId:patronId};$.getJSON(url,params,function(data){VuFind.showMessage(data.success?"Success":"Error",data.message,data.success,data.success)}).fail(VuFind.ajaxFail)})});return false}}}(VuFind.Hoopla||{});
VuFind.OverDrive=function(){return{cancelOverDriveHold:function(patronId,overdriveId){VuFind.confirm("Are you sure you want to cancel this hold?",function(){var ajaxUrl="/OverDrive/AJAX?method=CancelOverDriveHold&patronId="+patronId+"&overDriveId="+overdriveId;$.getJSON(ajaxUrl,function(data){if(data.success){VuFind.showMessage("Hold Cancelled",data.message,true);$("#overDriveHold_"+overdriveId).hide()}else{VuFind.showMessage("Error Cancelling Hold",data.message,false)}}).fail(function(){VuFind.showMessage("Error Cancelling Hold","An error occurred processing your request in OverDrive.  Please try again in a few minutes.",false)})});return false},checkOutOverDriveTitle:function(overDriveId){VuFind.Account.ajaxLogin(function(){var url="/OverDrive/"+overDriveId+"/AJAX?method=GetOverDriveCheckoutPrompts";$.getJSON(url,function(data){if(data.promptNeeded){VuFind.showMessageWithButtons(data.promptTitle,data.prompts,data.buttons)}else{VuFind.OverDrive.doOverDriveCheckout(data.patronId,overDriveId)}}).fail(function(){VuFind.showMessage("Error Checking Out Title","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})});return false},processOverDriveCheckoutPrompts:function(){var overdriveCheckoutPromptsForm=$("#overdriveCheckoutPromptsForm"),patronId=$("#patronId").val(),overdriveId=overdriveCheckoutPromptsForm.find("input[name=overdriveId]").val();VuFind.OverDrive.doOverDriveCheckout(patronId,overdriveId)},doOverDriveCheckout:function(patronId,overdriveId){VuFind.Account.ajaxLogin(function(){var ajaxUrl="/OverDrive/AJAX?method=CheckoutOverDriveItem&patronId="+patronId+"&overDriveId="+overdriveId;$.getJSON(ajaxUrl,function(data){if(data.success){VuFind.showMessageWithButtons("Title Checked Out Successfully",data.message,data.buttons)}else if(data.noCopies){VuFind.confirm(data.message,function(){VuFind.OverDrive.placeOverDriveHold(overdriveId,null)})}else{VuFind.showMessage("Error Checking Out Title",data.message,false)}}).fail(function(){VuFind.showMessage("Error Checking Out Title","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})});return false},doOverDriveHold:function(patronId,overDriveId,overdriveEmail,promptForOverdriveEmail){var url="/OverDrive/AJAX",params={method:"PlaceOverDriveHold",patronId:patronId,overDriveId:overDriveId,overdriveEmail:overdriveEmail,promptForOverdriveEmail:promptForOverdriveEmail};$.getJSON(url,params,function(data){if(data.availableForCheckout){VuFind.OverDrive.checkOutOverDriveTitle(overdriveId)}else{VuFind.showMessage("Placed Hold",data.message,true)}}).fail(function(){VuFind.showMessage("Error Placing Hold","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})},followOverDriveDownloadLink:function(patronId,overDriveId,formatId){var ajaxUrl="/OverDrive/AJAX?method=GetDownloadLink&patronId="+patronId+"&overDriveId="+overDriveId+"&formatId="+formatId;$.getJSON(ajaxUrl,function(data){if(data.success){var win=window.open(data.downloadUrl,"_blank");win.focus()}else{VuFind.showMessage("Error Getting Download Link",data.message,false)}}).fail(function(){VuFind.showMessage("Error Getting Download Link","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})},forceUpdateFromAPI:function(overDriveId){var url="/OverDrive/"+overDriveId+"/AJAX?method=forceUpdateFromAPI";$.getJSON(url,function(data){VuFind.showMessage("Success",data.message,true,true)});return false},placeOverDriveHold:function(overDriveId){VuFind.Account.ajaxLogin(function(){var url="/OverDrive/"+overDriveId+"/AJAX?method=GetOverDriveHoldPrompts";$.getJSON(url,function(data){if(data.promptNeeded){VuFind.showMessageWithButtons(data.promptTitle,data.prompts,data.buttons)}else{VuFind.OverDrive.doOverDriveHold(data.patronId,overDriveId,data.overdriveEmail,data.promptForOverdriveEmail)}}).fail(function(){VuFind.showMessage("Error Placing Hold","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})});return false},processOverDriveHoldPrompts:function(){var overdriveHoldPromptsForm=$("#overdriveHoldPromptsForm"),overdriveEmail=overdriveHoldPromptsForm.find("input[name=overdriveEmail]").val(),overdriveId=overdriveHoldPromptsForm.find("input[name=overdriveId]").val(),patronId=$("#patronId").val(),promptForOverdriveEmail;if(overdriveHoldPromptsForm.find("input[name=promptForOverdriveEmail]").is(":checked")){promptForOverdriveEmail=0}else{promptForOverdriveEmail=1}VuFind.OverDrive.doOverDriveHold(patronId,overdriveId,overdriveEmail,promptForOverdriveEmail)},returnOverDriveTitle:function(patronId,overDriveId,transactionId){VuFind.confirm("Are you sure you want to return this title?",function(){VuFind.showMessage("Returning Title","Returning your title in OverDrive.  This may take a minute.");var ajaxUrl="/OverDrive/AJAX?method=ReturnOverDriveItem&patronId="+patronId+"&overDriveId="+overDriveId+"&transactionId="+transactionId;$.getJSON(ajaxUrl,function(data){VuFind.showMessage("Title Returned",data.message,data.success,data.success)}).fail(function(){VuFind.showMessage("Error Returning Title","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})});return false},selectOverDriveDownloadFormat:function(patronId,overDriveId){var selectedOption=$("#downloadFormat_"+overDriveId+" option:selected"),selectedFormatId=selectedOption.val(),selectedFormatText=selectedOption.text();if(selectedFormatId===-1){alert("Please select a format to download.")}else{VuFind.confirm("Are you sure you want to download the "+selectedFormatText+" format? You cannot change format after downloading.",function(){var ajaxUrl="/OverDrive/AJAX?method=SelectOverDriveDownloadFormat&patronId="+patronId+"&overDriveId="+overDriveId+"&formatId="+selectedFormatId;$.getJSON(ajaxUrl,function(data){if(data.success){window.location.href=data.downloadUrl}else{VuFind.showMessage("Error Selecting Format",data.message)}}).fail(function(){VuFind.showMessage("Error Selecting Format","An error occurred processing your request in OverDrive.  Please try again in a few minutes.")})})}return false},submitHelpForm:function(){$.post("/OverDrive/AJAX?method=submitSupportForm",$("#eContentSupport").serialize(),function(data){VuFind.showMessage(data.title,data.message)},"json").fail(VuFind.ajaxFail);return false}}}(VuFind.OverDrive||{});
VuFind.Prospector=function(){return{getProspectorResults:function(prospectorNumTitlesToLoad,prospectorSavedSearchId){var url="/Search/AJAX",params={method:"getProspectorResults",prospectorNumTitlesToLoad:prospectorNumTitlesToLoad,prospectorSavedSearchId:prospectorSavedSearchId};$.get(url,params,function(data){$("#prospectorSearchResultsPlaceholder").html(data)})},loadRelatedProspectorTitles:function(id){var url="/GroupedWork/"+encodeURIComponent(id)+"/AJAX",params={method:"getProspectorInfo"};$.getJSON(url,params,function(data){if(data.numTitles===0){$("#prospectorPanel").hide()}else{$("#inProspectorPlaceholder").html(data.formattedData)}})},removeBlankThumbnail:function(imgElem,elemToHide,isForceRemove){var $img=$(imgElem);if($img.height()==1&&$img.width()==1||isForceRemove){$(elemToHide).remove()}}}}(VuFind.Prospector||{});
VuFind.Ratings=function(){$(function(){VuFind.Ratings.initializeRaters()});return{initializeRaters:function(){$(".rater").each(function(){var ratingElement=$(this),userRating=ratingElement.data("user_rating"),id=ratingElement.data("id"),options={id:id,rating:parseFloat(userRating>0?userRating:ratingElement.data("average_rating")),url:"/GroupedWork/"+encodeURIComponent(id)+"/AJAX"};ratingElement.rater(options)})},doRatingReview:function(id){$.getJSON("/GroupedWork/"+id+"/AJAX?method=getPromptforReviewForm",function(data){if(data.prompt)VuFind.showMessageWithButtons(data.title,data.modalBody,data.modalButtons);if(data.error)VuFind.showMessage("Error",data.message)}).fail(VuFind.ajaxFail)},doNoRatingReviews:function(){$.getJSON("/GroupedWork/AJAX?method=setNoMoreReviews",function(data){if(data.success)VuFind.showMessage("Success","You will no longer be asked to give a review.",true);else VuFind.showMessage("Error","Failed to save your setting.")}).fail(VuFind.ajaxFail)}}}(VuFind.Ratings);$.fn.rater=function(options){var opts=$.extend({},$.fn.rater.defaults,options);return this.each(function(){var $this=$(this),$on=$this.find(".ui-rater-starsOn"),$off=$this.find(".ui-rater-starsOff");if(opts.size==undefined)opts.size=$off.height();if(opts.rating==undefined){opts.rating=$on.width()/$off.width()}else{$on.width($off.width()*(opts.rating/opts.ratings.length))}if(opts.id==undefined)opts.id=$this.attr("id");var initialRating=opts.rating;if(!$this.hasClass("ui-rater-bindings-done")){$this.addClass("ui-rater-bindings-done");$off.mousemove(function(e){var left=e.clientX-$off.offset().left,width=$off.width()-($off.width()-left);width=Math.min(Math.ceil(width/(opts.size/opts.step))*opts.size/opts.step,opts.size*opts.ratings.length);$on.width(width);var r=Math.round($on.width()/$off.width()*(opts.ratings.length*opts.step))/opts.step;$this.attr("title",'Click to Rate "'+r+' stars"')}).hover(function(e){$on.addClass("ui-rater-starsHover")},function(e){$on.removeClass("ui-rater-starsHover");$on.width(initialRating*opts.size)}).click(function(e){var r=Math.round($on.width()/$off.width()*(opts.ratings.length*opts.step))/opts.step;$.fn.rater.rate($this,opts,r)}).css("cursor","pointer");$on.css("cursor","pointer")}})};$.fn.rater.defaults={url:location.href,ratings:["Hated It","Didn't Like It","Liked It","Really Liked It","Loved It"],step:1};$.fn.rater.rate=function($this,opts,rating){VuFind.Account.ajaxLogin(function(){var $on=$this.find(".ui-rater-starsOn"),$off=$this.find(".ui-rater-starsOff");$off.fadeTo(600,.4,function(){$.getJSON(opts.url,{method:"RateTitle",id:opts.id,rating:rating},function(data){if(data.error){VuFind.showMessage("Error",data.error);$off.fadeTo(500,1).mouseleave()}if(data.rating){opts.rating=data.rating;$off.fadeTo(600,.1,function(){$on.removeClass("ui-rater-starsHover").width(opts.rating*opts.size).addClass("userRated");$off.fadeTo(500,1);$this.attr("title","Your rating: "+rating.toFixed(1));if($this.data("show_review")==true){VuFind.Ratings.doRatingReview(opts.id)}})}}).fail(function(){VuFind.ajaxFail();$off.fadeTo(500,1).mouseleave()})})},null,true)};
VuFind.Account.ReadingHistory=function(){return{deletedMarkedAction:function(){VuFind.confirm("The marked items will be irreversibly deleted from your reading history.  Proceed?",function(){$("#readingHistoryAction").val("deleteMarked");$("#readingListForm").submit()});return false},deleteAllAction:function(){VuFind.confirm("Your entire reading history will be irreversibly deleted.  Proceed?",function(){$("#readingHistoryAction").val("deleteAll");$("#readingListForm").submit()});return false},optOutAction:function(){VuFind.confirm("Opting out of Reading History will also delete your entire reading history irreversibly.  Proceed?",function(){$("#readingHistoryAction").val("optOut");$("#readingListForm").submit()});return false},optInAction:function(){$("#readingHistoryAction").val("optIn");$("#readingListForm").submit();return false},exportListAction:function(){$("#readingHistoryAction").val("exportToExcel");$("#readingListForm").submit();return false}}}(VuFind.Account.ReadingHistory||{});
...
VuFind.Wikipedia=function(){return{getWikipediaArticle:function(articleName){var url="/Author/AJAX?method=getWikipediaData&articleName="+articleName;$.getJSON(url,function(data){if(data.success)$("#wikipedia_placeholder").html(data.formatted_article).fadeIn()})}}}(VuFind.Wikipedia);
