				...
				<strong>{translate text='Search Tools'}:</strong>
					{if $showSearchTools}
						...
						&nbsp;&nbsp;<a class="text-nowrap" href="#" onclick="return Pika.Account.ajaxLightbox('/Search/AJAX?method=getEmailForm');"><span class="bi bi-envelope" aria-hidden="true"></span>&nbsp;{translate text='Email this Search'}</a>
							{if $loggedIn && $savedSearch}
								{* Do not offer to delete saved search if not logged in. *}
								&nbsp;&nbsp;<a class="text-nowrap" href="#" onclick="return Pika.Account.deleteSearch('{$searchId}',1,1)"><span class="bi bi-trash" aria-hidden="true"></span>&nbsp;{translate text='save_search_remove'}</a>
							{else}
