{if $cluster.showMoreFacetPopup}
	{foreach from=$cluster.list item=thisFacet name="narrowLoop"}
		{if $thisFacet.isApplied}
			<div class="facetValue"><img src="/images/silk/tick.png" alt="Selected" /> {$thisFacet.display|escape} <a href="{$thisFacet.removalUrl|escape}" class="removeFacetLink" onclick="trackEvent('Remove Facet', '{$cluster.label}', '{$thisFacet.display|escape}');">(remove)</a></div>
		{else}
	...
	{/foreach}
	{* Show more list *}
	<div class="facetValue" id="more{$title}"><a href="#" onclick="Pika.ResultsList.moreFacetPopup('More options for {$cluster.label}', '{$title}'); return false;">{translate text='more'} ...</a></div>
...
{else}
	{foreach from=$cluster.list item=thisFacet name="narrowLoop"}
		...
		{* Show More link*}
			<div class="facetValue" id="more{$title}"><a href="#" onclick="Pika.ResultsList.moreFacets('{$title}'); return false;">{translate text='more'} ...</a></div>
	...
	{if $smarty.foreach.narrowLoop.total > $cluster.valuesToShow}
		<div class="facetValue">
			<a href="#" onclick="Pika.ResultsList.lessFacets('{$title}'); return false;">{translate text='less'} ...</a>
		</div>
