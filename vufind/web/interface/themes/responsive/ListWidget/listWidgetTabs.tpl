		...
		{if !isset($widget->listDisplayType) || $widget->listDisplayType == 'tabs'}
			...
			<ul class="nav nav-tabs" role="tablist">
				{foreach from=$widget->lists item=list name=listWidgetList}
					...
					<li {if $active}class="active"{/if} role="tab" data-toggle="tab" data-index="{$smarty.foreach.listWidgetList.index}">
						<a href="#list-{$list->name|regex_replace:'/\W/':''|escape:url}">{$list->name}</a>
					</li>
					{/if}
				{/foreach}
			</ul>
			...
			{foreach from=$widget->lists item=list name=listLoop}
				...
				{if $list->displayFor == 'all' || ($list->displayFor == 'loggedIn' && $loggedIn) || ($list->displayFor == 'notLoggedIn' && !$loggedIn)}
					{if $index == 0}
						...
						listScroller{$listName}.loadTitlesFrom('/Search/AJAX?method=GetListTitles%26id={$list->source|escape:url}%26scrollerName={$listName}%26coverSize={$widget->coverSize}%26showRatings={$widget->showRatings}%26numTitlesToShow={$widget->numTitlesToShow}{if $reload}%26reload=true{/if}', false);
					{/if}
