			...
			{foreach from=$widget->lists item=list name=listLoop}
				...
				{if $list->displayFor == 'all' || ($list->displayFor == 'loggedIn' && $loggedIn) || ($list->displayFor == 'notLoggedIn' && !$loggedIn)}
					{if $index == 0}
						...
						listScroller{$listName}.loadTitlesFrom('/API/ListAPI?method=getListWidgetTitles%26id={$list->source|escape:url}%26scrollerName={$listName}%26coverSize={$widget->coverSize}%26showRatings={$widget->showRatings}%26numTitlesToShow={$widget->numTitlesToShow}{if $reload}%26reload=true{/if}{if $listWidgetToken}%26{$listWidgetToken}{/if}', false);
					{/if}
