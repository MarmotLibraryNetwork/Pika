...
<div id="list-{$wrapperId}"{if $display == 'false'} style="display:none"{/if} class="verticalTitleScroller{if $widget->coverSize == 'medium'} mediumScroller{/if}">
	<div id="{$wrapperId}" class="titleScrollerWrapper">
		...
		<div id="titleScroller{$scrollerName}" class="titleScrollerBody">
			<div class="scrollerButtonUp btn btn-primary" onclick="{$scrollerVariable}.scrollToLeft();"><i class="bi bi-chevron-up"></i></div>
			<div class="scrollerBodyContainer">
				<div class="scrollerBody" style="display:none"></div>
				<div class="scrollerLoadingContainer">
					<img id="scrollerLoadingImage{$scrollerName}" class="scrollerLoading" src="{img filename="loading_large.gif"}{if $listWidgetToken}?{$listWidgetToken}{/if}" alt="Loading...">
				</div>
			</div>
			<div class="clearer"></div>
			<div class="scrollerButtonDown btn btn-primary" onclick="{$scrollerVariable}.scrollToRight();"><i class="bi bi-chevron-down"></i></div>
		</div>
	</div>
</div>
<script>
