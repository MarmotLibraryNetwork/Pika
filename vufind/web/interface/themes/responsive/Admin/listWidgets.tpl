			...
			{foreach from=$availableWidgets key=id item=widget}
				<tr><td>{$widget->id}</td><td>{$widget->name}</td><td>{$widget->getLibraryName()}</td><td>{$widget->description}</td><td>
					<div class="btn-group-vertical btn-group-sm">
						<a class="btn btn-sm btn-outline-secondary" href="/Admin/ListWidgets?objectAction=view&id={$widget->id}" role="button">View</a>
						<a class="btn btn-sm btn-outline-secondary" href="/Admin/ListWidgets?objectAction=edit&id={$widget->id}" role="button">Edit</a>
						<a class="btn btn-sm btn-outline-secondary" href="/API/SearchAPI?method=getListWidget&id={$widget->id}" role="button">Preview</a>
						{if $canDelete}
