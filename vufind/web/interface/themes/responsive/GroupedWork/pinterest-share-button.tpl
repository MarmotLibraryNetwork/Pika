{strip}
	<a {if $linkClass}class="{$linkClass}" {/if}href="http://www.pinterest.com/pin/create/button/?url={$urlToShare}{if $coverUrl}&media={$coverUrl|escape:'url'}{/if}{if $description}&description={$description|escape:'url'}{/if}" {* keep space between attributes *}
