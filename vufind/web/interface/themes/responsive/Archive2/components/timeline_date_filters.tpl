...
{strip}
	{if $dateFacetInfo || $unknownDateCount}
		<div class="btn-group btn-group-sm timeline-date-filters" role="group" aria-label="Filter by Date">
			<button type="button" class="btn btn-outline-secondary btn-sm{if !$selectedDateFilter} active{/if}" onclick="Pika.Archive2.setTimelineDateFilter('', this)">
				<strong>All</strong><br>({$recordCount})
			</button>
			{foreach from=$dateFacetInfo item=facet}
				<button type="button" class="btn btn-outline-secondary btn-sm{if $selectedDateFilter == $facet.value} active{/if}" onclick="Pika.Archive2.setTimelineDateFilter('{$facet.value}', this)">
					<strong>{$facet.label}</strong><br>({$facet.count})
				</button>
			{/foreach}
			{if $unknownDateCount > 0}
				<button type="button" class="btn btn-outline-secondary btn-sm{if $selectedDateFilter == 'unknown'} active{/if}" onclick="Pika.Archive2.setTimelineDateFilter('unknown', this)">
