				...
				{if !$masqueradeMode || ($masqueradeMode && $allowReadingHistoryDisplayInMasqueradeMode)}
					...
					<div class="row">
						...
						{if $transList || $isReadingHistorySearch}
							...
							{if $pageLinks.all}<nav class="d-flex justify-content-center" aria-label="Reading history pages, top">{$pageLinks.all}</nav>{/if}
							...
							{if $historyActive == true && $isReadingHistorySearch == true && count($transList) < 1}
							...
							{if $pageLinks.all}<nav class="d-flex justify-content-center" aria-label="Reading history pages, bottom">{$pageLinks.all}</nav>{/if}
