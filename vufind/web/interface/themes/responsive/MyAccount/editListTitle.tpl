...
<div id="page-content" class="content">
	{if $currentListURL}
		<a href="{$currentListURL|escape:"html"}" class="btn btn-default btn-sm">{translate text='Return to List'}</a>
	{/if}
	<div id="main-content">
		{if $error}
			<div class="alert alert-danger">{$error}</div>
		{else}
			<h1 role="heading" aria-level="1" class="h2" id="resourceTitle">{$recordDriver->getTitle()|escape:"html"}</h1>
