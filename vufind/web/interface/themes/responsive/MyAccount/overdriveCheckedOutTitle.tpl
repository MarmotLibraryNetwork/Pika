						...
						{if $record.coverUrl}
							{if $record.recordId && $record.linkUrl}
								<a href="{$record.linkUrl}" id="descriptionTrigger{$record.recordId|escape:"url"}">
									<img src="{$record.coverUrl}" class="listResultImage img-thumbnail img-responsive" alt="{if !$record.title}Cover image for Overdrive checkout.{else}Cover image for {$record.title}{/if}.">
								</a>
			...
			<div class="resultDetails col-sm-12 col-lg-9">
				{if strlen($record.author) > 0}
					<div class="row">
						...
						<div class="result-value col-8 col-xl-9">{$record.author}</div>
					</div>
				{/if}

				<div class="row">
					<div class="result-label col-4 col-xl-3">{translate text='Source'}</div>
					<div class="result-value col-8 col-xl-9">{$record.checkoutSource}</div>
				</div>
				...
				{if $record.edition}
					<div class="row">
						...
						<div class="result-label col-4 col-xl-3">{translate text='Edition'}</div>
						<div class="result-value col-8 col-xl-9">{$record.edition}</div>
					</div>
				{/if}
				...
				{if $record.isFormatSelected}
					<div class="row">
						...
						<div class="result-value col-8 col-xl-9">{$record.selectedFormat.name}</div>
					</div>
				{/if}
...
{*					<div class="row">*}
{*						<div class="result-label col-4 col-xl-3">Rating&nbsp;</div>*}
...
{*				{/if}*}
				...
				<div class="row">
					<div class="result-label col-4 col-xl-3">{translate text='Expires'}</div>
					<div class="result-value col-8 col-xl-9">{$record.dueDate|date_format}</div>
				</div>
