						...
						{if $record.coverUrl}
							{if $record.recordId && $record.linkUrl}
								<a href="{$record.linkUrl}" id="descriptionTrigger{$record.recordId|escape:"url"}">
									<img src="{$record.coverUrl}" class="listResultImage img-thumbnail img-fluid" alt="{if !$record.title}Cover image for Overdrive checkout.{else}Cover image for {$record.title}{/if}.">
								</a>
