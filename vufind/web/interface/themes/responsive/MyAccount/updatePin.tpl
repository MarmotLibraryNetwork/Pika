	...
	<form action="/MyAccount/UpdatePin" method="post" class="form-horizontal" id="pinForm">
		<div class="form-group">
            ...
            {* No size limits in case previously set password doesn't meet current restrictions *}
		        ...
		        <button aria-label="show {translate text='PIN'}" onclick="$('span', this).toggle(); $(this).attr('aria-label',$(this).children('span:visible').children('div').text()); return Pika.pwdToText('pin');" class="btn btn-outline-secondary" type="button" ><span class="glyphicon glyphicon-eye-open" aria-hidden="true" title="Show {translate text='PIN'}"><div class="hiddenText">Show {translate text='PIN'}</div></span><span class="glyphicon glyphicon-eye-close" style="display: none" title="Hide {translate text='PIN'}"><div class="hiddenText">Hide {translate text='PIN'}</div></span></button>
		      </span>
				</div>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-8">
				<div class="input-group">
					...
					<span class="input-group-btn" style="vertical-align: top"{* Override so button stays in place when input requirement message displays *}>
		        <button aria-label="show {translate text='PIN'}" onclick="$('span', this).toggle(); $(this).attr('aria-label',$(this).children('span:visible').children('div').text()); return Pika.pwdToText('pin1')" class="btn btn-outline-secondary" type="button"><span class="glyphicon glyphicon-eye-open" title="Show {translate text='PIN'}"><div class="hiddenText">Show {translate text='PIN'}</div></span><span class="glyphicon glyphicon-eye-close" style="display: none" title="Hide {translate text='PIN'}"><div class="hiddenText">Hide {translate text='PIN'}</div></span></button>
		      </span>
				</div>
			</div>
		</div>
