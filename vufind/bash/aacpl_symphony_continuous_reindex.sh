...
do
	...
	find /mnt/ftp/symphony-updates -maxdepth 1 -mmin -60 -name '*.mrc' | while IFS= read -r FILE; do
		#Above find is for test only. Copy any partial exports from the last 60 minutes because of the moving out the partials is only done in production

		#find /mnt/ftp/symphony-updates -maxdepth 1 -name '*.mrc' | while IFS= read -r FILE; do
...
#			echo "$FILE was modified less than 1 minute ago, waiting to copy " >> "${OUTPUT_FILE}"
	...
	cd "/data/pika/${PIKASERVER}/marc" || exit
...
#	wget -q "https://drive.google.com/uc?export=download&id=0B_xqNQMfUrAzNGJrajJzQWs3ZGs" -O "Pika_Holds.csv"
	...
	cd /usr/local/pika/vufind/symphony_export/ || exit
