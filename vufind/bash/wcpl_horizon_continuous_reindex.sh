...
do
	...
	find /mnt/ftp/continuous_exports -maxdepth 1 -mmin -30 -name '*.mrc' | while IFS= read -r FILE; do
		...
		#find /mnt/ftp/continuous_exports -maxdepth 1 -name '*.mrc' | while IFS= read -r FILE; do
...
#			echo "$FILE was modified less than 1 minute ago, waiting to copy " >> "${OUTPUT_FILE}"
	...
	if test "$(find "/data/pika/${PIKASERVER}/marc_updates/" -name "*.mrc" -mtime +1)"; then
		{
			echo "Partial Exports older than a day found in marc_updates folder. Deleting."
			echo ""
			find "/data/pika/${PIKASERVER}/marc_updates/" -name "*.mrc" -mtime +1
		} >> "${OUTPUT_FILE}"
	...
	cd /usr/local/pika/vufind/horizon_export/ || exit
