...
//			PreparedStatement holdsToProcessStmt = pikaConn.prepareStatement("SELECT offline_hold.*, cat_username, cat_password FROM `offline_hold` LEFT JOIN `user` ON (user.barcode = offline_hold.patronBarcode) WHERE status = 'Not Processed' ORDER BY timeEntered ASC, id ASC");
	...
	private void processOfflineCirculationEntryViaSierraAPI(PreparedStatement updateCirculationEntry, String sierraApiUrl, ResultSet circulationEntriesToProcessRS) throws SQLException {
		...
		OfflineCirculationResult result        = processOfflineCheckout(sierraApiUrl, login, itemBarcode, patronBarcode, patronPikaId);
		if (result.isSuccess()){
			processLog.incUpdated();
			updateCirculationEntry.setString(2, "Processing Succeeded");
		}else{
...
//	private void processOfflineCirculationEntryViaSierraAPI(PreparedStatement updateCirculationEntry, String sierraApiUrl, ResultSet circulationEntriesToProcessRS, PreparedStatement patronPinStmt) throws SQLException {
//		long circulationEntryId = circulationEntriesToProcessRS.getLong("id");
//		updateCirculationEntry.clearParameters();
//		updateCirculationEntry.setLong(1, new Date().getTime() / 1000);
//		updateCirculationEntry.setLong(4, circulationEntryId);
...
//		String                   login         = circulationEntriesToProcessRS.getString("login");
//		String                   patronBarcode = circulationEntriesToProcessRS.getString("patronBarcode");
//		OfflineCirculationResult result        = processOfflineCheckout(sierraApiUrl, login, itemBarcode, patronBarcode, patronPinStmt);
//		if (result.isSuccess()){
//			processLog.incUpdated();
//			updateCirculationEntry.setString(2, "Processing Succeeded");
//		}else{
...
//					} else if (patronBarcodeResponse.isSuccess() && patronBarcodeResponse.getMessage().contains("<h[123] class=\"error\">")) {
			...
			result.setSuccess(false);
			result.setNote("Unexpected error processing check in " + e);
		}
...
//
//	private OfflineCirculationResult processOfflineCheckout(String baseSierraApiUrl, String sierraCircLogin, String itemBarcode, String patronBarcode, PreparedStatement patronPinStmt) {
//		OfflineCirculationResult result             = new OfflineCirculationResult();
//		String                   checkoutUrl        = baseSierraApiUrl + "/patrons/checkout";
//		String                   encryptedPatronPin = "";
//		String                   pin                = "";
//		try {
//			try {
//				patronPinStmt.setString(1, patronBarcode);
//				ResultSet patronPinResult = patronPinStmt.executeQuery();
//				if (patronPinResult.next()){
//					encryptedPatronPin = patronPinResult.getString(1);
//					if (patronPinResult.next()){
//						logger.warn("Multiple entries for user barcode " + patronBarcode);
//					}
//					//TODO: only need to set key once
//					byte[] key = Base64.getMimeDecoder().decode(PikaConfigIni.getIniValue("Site", "passwordEncryptionKey"));
////					System.out.println(key);
////					System.out.println(new String(key));
//					String string = new String(Base64.getMimeDecoder().decode(encryptedPatronPin));
//					System.out.println(string);
//
//					String[] parts = string.split("::");
//					encryptedPatronPin = parts[0];
...
//
//						byte[] iv = new byte[16];
//						byte[] bytes = initVector.getBytes(StandardCharsets.US_ASCII);
//						for (int i=0;i<15;i++){
//							iv[i] = bytes[i];
//						}
...
//						plaintext = cipher.doFinal(encryptedPatronPin.getBytes(StandardCharsets.UTF_8));
//						pin = Arrays.toString(plaintext);
//					}catch(Exception e){
//						logger.error("Decrypting patron pin", e);
//					}
//
//
//				}
//			} catch (SQLException e) {
//          logger.error("Error fetching patron pin", e);
//      }
//
//
//			 JSONObject json = new JSONObject()
...
//						Pattern Regex = Pattern.compile("<h3 class=\"error\">(.*?)</h3>", Pattern.CANON_EQ);
	 ...
	 * @param patronBarcode barcode to hash with the userApiToken
	 * @return Hash to use as url token for User API calls that support tokens
	 */
	private String md5(String patronBarcode){
		StringBuilder md5 = new StringBuilder();
		try {
			patronBarcode = patronBarcode + userApiToken;
			MessageDigest messageDigest = MessageDigest.getInstance("MD5");
			messageDigest.reset();
			messageDigest.update(patronBarcode.getBytes(StandardCharsets.UTF_8));
