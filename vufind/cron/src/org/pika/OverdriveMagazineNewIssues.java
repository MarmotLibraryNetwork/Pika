...
public class OverdriveMagazineNewIssues implements IProcessHandler{
			...
			if(clientSecret == null || clientKey == null || clientSecret.length() == 0 || clientKey.length() == 0 || accountIds.isEmpty())
			{
				...
				if(!magazines.wasNull()) {
					while (magazines.next()) {
						getNewMagazineIssuesById(magazines.getString("overdriveId"), magazines.getString("crossRefId"),logger, econtentConn);
	...
	public void getNewMagazineIssuesById(String overdriveId, String crossRefId, Logger logger, Connection econtentConn)
	{
		try {
			String overDriveUrl = "https://api.overdrive.com/v1/collections/"+ overDriveProductsKeys.firstEntry().getValue() + "/products/" + overdriveId + "/issues?limit=25&sort=saledate:dsc";
			...
			for(int i = 0; i< products.length(); i++)
			{
				...
				if(!idResult.first()){
					...
					//TODO: stop process when we find an existing issue
				}
			}
		}catch(Exception e)
		{
